...
public class CrawlerThread {
	...
	public CrawlerThread() {
		 ...
		 * databaseName; try{
		 * 
		 * DriverManager.registerDriver(new OracleDriver()); //ods = new
		 * OracleDataSource(); //ods.setURL(jdbcUrl); conn1 =
		 * DriverManager.getConnection(jdbcUrl,username,password); conn2 =
		 * DriverManager.getConnection(jdbcUrl,username,password);
		 * 
		 * //reset the database CallableStatement resetStmt = conn1.prepareCall(
		 * "begin RESETDB; end;"); resetStmt.executeUpdate(); resetStmt.close();
		 * 
		 * } catch(Exception e){
		 * 
	...
	synchronized public boolean addToList(HashSet Set, String link) {
		 ...
		 * if(!link.contains("'")){
		 * 
		 * if(tableName.equals("newlinks")) query = "INSERT INTO "+tableName+
		 * " (link,visit,linknumber) VALUES('"+link+"',0,linknumber.nextval)";
		 * else if(tableName.equals("oldlinks")) query = "INSERT INTO "
		 * +tableName+" (link) VALUES('"+link+"')"; else query = "";
		 * 
		 * }
		 * 
		 * else return ; try{
		 ...
		 * 
		 * } stmt = conn1.createStatement(); stmt.executeUpdate(query);
		 * 
		 * 
		 * } catch(Exception e){
		 * 
	...
	synchronized public boolean isNewLink(ArrayList<HashSet> s, String link) {
	...
	synchronized public String getNextLink(HashSet set) {
		 ...
		 * 
		 * //retrieve the first row String query = "SELECT link FROM "
		 * +tableName+" WHERE linknumber = "+rowId+ " AND visit = 0"; rowId ++;
		 * try{
		 * 
		 ...
		 * while(row.next()){
		 * 
		 * link = row.getString(1); //row.deleteRow();
		 * 
		 * } String del_query = "UPDATE newlinks SET visit = 1  WHERE link = '"
		 * +link+"'"; stmt.executeUpdate(del_query);
		 * 
		 * 
		 * } catch(Exception e){
		 * 
