...
private String seed;
private LinkedHashSet newLinks = new LinkedHashSet();//to preserve the order of insertion
...
//check to see if link that we are looking for exists in the link list
...
System.out.println("Thread "+Thread.currentThread().getId()+": "+currentLink+" ---> "+global.getLink().get(h).toString());
...
//check to see if links are new or not, and if they are new, insert into newLinks
...
//System.out.println(links.get(i)+"  " +Thread.currentThread().getId());
